import { VehicleType, FareCalculationResult } from './types';
...
// Pre-defined simulated distance matrix between key locations in km
...
/**
 * Calculates estimated distance in km between any two points
 */
export function getEstimatedDistance(pickup: string, drop: string): number {
  ...
  // Deterministic pseudo-distance for custom input
  let hash = 0;
  ...
  const distance = ((positiveHash % 180) / 10) + 2.5; // between 2.5 and 20.5 km
 ...
 */
export function calculateFare(vehicleType: VehicleType, distanceKm: number): FareCalculationResult {
  ...
  // Approximate average speed: Car 25 km/h, Bike 30 km/h in city
...
export function generateTxnId(method: string): string {
  const prefix = method === 'UPI' ? 'TXN_UPI' : method === 'Card' ? 'TXN_CRD' : 'TXN_CSH';
